...
@section('title', 'Modify FAQ Details')
        ...
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <select name="category" class="form-select @error('category') is-invalid @enderror" required>
              <option value="general" {{ old('category', $faq->category) === 'general' ? 'selected' : '' }}>General (Writing &amp; Illustrating)</option>
              <option value="shipping" {{ old('category', $faq->category) === 'shipping' ? 'selected' : '' }}>Shipping &amp; Timelines</option>
              <option value="refunds" {{ old('category', $faq->category) === 'refunds' ? 'selected' : '' }}>Cancellations &amp; Corrections</option>
            </select>
            @error('category')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          ...
          <div class="col-md-4">
            ...
            <input type="number" name="display_order" class="form-control @error('display_order') is-invalid @enderror" value="{{ old('display_order', $faq->display_order) }}" required min="0">
            @error('display_order')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
