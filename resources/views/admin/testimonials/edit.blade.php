...
@section('title', 'Modify Testimonial Details')
      ...
      <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <input type="text" name="designation" class="form-control @error('designation') is-invalid @enderror" value="{{ old('designation', $testimonial->designation) }}" placeholder="e.g. Gifted to her parents, Anniversaries">
            @error('designation')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-4">
            ...
            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
              <option value="1" {{ old('status', $testimonial->status ? '1' : '0') === '1' ? 'selected' : '' }}>Active</option>
              <option value="0" {{ old('status', $testimonial->status ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive</option>
            </select>
            @error('status')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          ...
          <div class="col-md-8">
            ...
            @if($testimonial->image)
              <div class="mt-2 p-1 border rounded bg-light d-inline-block">
                <img src="{{ asset($testimonial->image) }}" height="50" class="rounded-circle" alt="">
              </div>
            @endif
          </div>
          ...
          <div class="col-12">
            ...
            <textarea name="review" class="form-control @error('review') is-invalid @enderror" rows="4" required>{{ old('review', $testimonial->review) }}</textarea>
            @error('review')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
