...
class BlogController extends Controller
{
    ...
    public function index()
    {
        ...
        $posts = $blogs;
        return view('admin.blog.index', compact('blogs', 'posts'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'publish_date_tag' => 'nullable|string|max:100',
     ...
     */
    private function composeArticle(Request $request): array
    {
        $data = $request->except(['featured_image_file', 'blocks', 'promo', 'sidebar_promo', 'title_html', '_token', '_method']);
        ...
        $data['publish_date_tag'] = $request->input('publish_date_tag') ?: 'UPDATED THIS MONTH';
    ...
    public function preview(Request $request)
    {
        ...
        return view('admin.blog.preview', [
            'headline'  => $titleHtml !== '' ? strip_tags($titleHtml, '<em><i>') : e($request->input('title', 'Untitled')),
            'dek'       => $request->input('dek'),
            'category'  => $request->input('category'),
            'readTime'  => $request->input('read_time') ?: $renderer->readTime($blocks),
            'body'      => $renderer->render($blocks, is_array($promo) ? $promo : null),
            'sidebar'   => $renderer->renderSidebar(is_array($sidebar) ? $sidebar : null),
            'toc'       => $request->boolean('show_toc') ? $renderer->tableOfContents($blocks) : [],
            'tocLabel'  => $request->input('toc_label') ?: 'On this page',
